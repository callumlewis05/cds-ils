...
@model.over("_is_electronic", "^007")
def physical_medium(self, key, value):
    """Check for electronic resource."""
    return bool(value) and value.strip().startswith("c")
...
def identifiers(self, key, value):
    ...
    material = "DIGITAL" if self.get("_is_electronic") else "PRINT_VERSION"
    for v in force_list(value):
        sub_a = clean_val("a", v, str)
        if sub_a:
            isbn_value = re.sub(r"\s*\(.*\)\s*$", "", sub_a).strip()
            isbn = {"value": isbn_value, "scheme": "ISBN", "material": material}
...
@model.over("copyright", "^264_4")
@filter_list_values
def copyright(self, key, value):
    """Translate copyright notice. Only year is contained in GOBI records."""
    _copyrights = self.get("copyright", [])
    notice = clean_val("c", value, str)
    if notice:
        numbers = re.findall(r"\d{4}", notice)
        if numbers:
            copyright_entry = {"year": int(numbers[0])}
            if copyright_entry not in _copyrights:
                _copyrights.append(copyright_entry)
    return _copyrights
...
def id_control_numbers(self, key, value):
    ...
    for cn in control_numbers:
        ...
        if match:
            ...
            alt_id = {"scheme": "GOBI", "value": cn.strip()}
